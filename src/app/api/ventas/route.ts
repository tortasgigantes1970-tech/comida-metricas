...
export async function GET(req: NextRequest) {
  try {
    ...
    let sql = `SELECT v.id, v.fecha, v.total, v.total_costo, v.notas, v.created_at
               FROM ventas v`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rVenta = await db.execute({
      sql: `INSERT INTO ventas (fecha, total, total_costo, notas) VALUES (?, ?, ?, ?)`,
      args: [fecha, total, total_costo, notas ?? ''],
    });
